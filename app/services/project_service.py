...
async def create_project(db: Session, user_id: str, zip_file: bytes, filename: str):
    ...
    project = Project(user_id=user_id, job_id=job_id, status="queued")  # Include user_id
    save_project(db, project)
    logger.info(f"Project created with job_id {job_id} for user {user_id}")
    return {"project_id": project.id, "user_id": user_id, "job_id": job_id, "status": "queued"}
